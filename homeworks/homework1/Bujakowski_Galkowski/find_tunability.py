...
def main(args):
            ...
            while True:
                try:
                    if model == 'RFC':
                        ...
                        new_hyperparam_value = np.random.choice(HYPERPARAMETERS_SPACE_LR[hyperparam])
                        best_hparams_copy = best_hparams_auc.copy()
                        best_hparams_copy[hyperparam] = new_hyperparam_value
                        clf = LogisticRegression(**best_hparams_copy)
                    elif model == 'XGB':
                        new_hyperparam_value = np.random.choice(HYPERPARAMETERS_SPACE_XGB[hyperparam])
                        best_hparams_copy = best_hparams_auc.copy()
                        best_hparams_copy[hyperparam] = new_hyperparam_value
    ...
    for hyperparam in hyperparams_tunability.keys():
        with open(f'tunability/{model}/{model}_{hyperparam}_tunability.pickle', 'wb') as handle:
